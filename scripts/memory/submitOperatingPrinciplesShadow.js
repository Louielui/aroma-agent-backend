...
/**
 * submitOperatingPrinciplesShadow — M3b-2 governed submission CLI.
 *
 * IMPORTANT: This CLI does NOT approve or activate anything. It builds the
 * canonical Operating Principles payload, creates exactly one append-only
 * revision, and emits exactly one SUBMITTED_FOR_REVIEW event — stopping in
 * derived state `review_ready`. Approval and activation are separate future
 * steps under separate Owner GOs. `--submission-ref` / `--rationale` are
 * submission-authorization + audit provenance only (NOT content approval).
 *
 * Safety: without --confirm this is a DRY RUN (no writes). `sourceCommit` is
 * derived from the verified M3a trust anchor (never operator input / branch HEAD).
 * A pre-existing matching event-less revision requires an EXACT --resume <id>
 * acknowledgement; otherwise RESUME_REQUIRED / exit 2 / zero writes. Never creates
 * a second revision to recover a partial attempt; no rollback, no silent repair.
 *
 ...
 *
 *   AROMA_CORE_DIR=<abs> node scripts/memory/submitOperatingPrinciplesShadow.js \
 *     --submission-ref <ref> --rationale "<why>" [--resume <revisionId>] \
 *     [--expect-source-commit <40hex>] --confirm
 */

const { resolveCoreDir } = require('../../src/core/memory/store')
const submit = require('../../src/core/memory/shadow/operatingPrinciplesSubmit')
...
function main (argv) {
  ...
  let result
  try {
    result = submit.submitOperatingPrinciples(baseDir, {
      personaIdentity,
