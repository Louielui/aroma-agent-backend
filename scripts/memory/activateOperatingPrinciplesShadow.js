...
/**
 * activateOperatingPrinciplesShadow — M3b-4 governed activation CLI.
 *
 * IMPORTANT: This CLI does NOT perform runtime cutover and does not itself
 * constitute activation authority — it records an explicit out-of-band Owner
 * activation GO. It emits exactly one ACTIVATED event on the single `approved`
 * operating-principles revision, advancing the derived state to `active` (resolver
 * ACTIVE) in the Memory truth layer only. Runtime (PERSONA_IDENTITY /
 ...
 * --expect-revision-id, --expect-source-commit, --activation-ref, --rationale are
 * REQUIRED. `sourceCommit` truth is the M3a anchor; --expect-source-commit is a
 * mandatory equality guard for --confirm. No --resume.
 *
 * Exit codes: 0 success (activated/already-active/dry-run) · 2 governance refusal /
 *             write failure · 3 config-tool / validation.
 *
 *   AROMA_CORE_DIR=<abs> node scripts/memory/activateOperatingPrinciplesShadow.js \
...
const activate = require('../../src/core/memory/shadow/operatingPrinciplesActivate')
...
function main (argv) {
  ...
  let result
  try {
    result = activate.activateOperatingPrinciples(baseDir, {
      personaIdentity,
      activationRef: args.activationRef,
