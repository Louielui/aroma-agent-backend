...
test('e2e: real personaCanary entrypoint serves /persona-canary/health and /readiness (200, safe)', async () => {
  ...
  try {
    ...
    for (const leak of ['canary-secret', '心燈', '思考順序', P.slice(0, 807), P.slice(886, 952), '/Users/', 'AromaCore', 'Error:', 'HUB_TOKEN', 'ANTHROPIC']) assert.equal(all.includes(leak), false, 'leak: ' + leak)
