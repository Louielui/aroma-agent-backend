...
function assertNoLeak (obj) {
  ...
  for (const leak of ['守燈', '思考順序', '表達風格', P.slice(0, 807), P.slice(807, 1586), P.slice(1586), 'HUB_TOKEN', 'ANTHROPIC', 'canary-secret', '/Users/', 'AromaCore', 'Error:', 'at Object']) assert.equal(s.includes(leak), false, 'leaked: ' + leak)
}
...
test('safeReason only echoes uppercase codes; anything else -> NOT_READY', () => {
  ...
  assert.equal(H.safeReason('守燈 leaked persona text'), 'NOT_READY')
