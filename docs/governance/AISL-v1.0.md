...
This set is APPROVED by Louie and is the authoritative AISL v1.0 governance constitution. No code,
...
- **v1.0 §6 correction — 2026-07-13.** Replaced the stale §6 "DRAFT until the Owner approves it" sentence with an APPROVED statement, and removed the now-dangling pre-approval clause ("Until approved it documents intent; it does not itself grant or revoke any capability.") as part of the same correction, so §6 reads cleanly while preserving the "No code, runtime… must conform to" governance content. Resolves the last DRAFT/APPROVED contradiction. Owner Governance GO (Owner-only, Governance Plane) issued by Louie — 2026-07-13 (initial + follow-up).
