...
function createGithubReadAdapter (options = {}) {
  ...
  const methods = {
    async listPullRequests ({ owner, repo, state = 'open', per_page = 25 } = {}) {
      ...
      return (r.data || []).map((pr) => makeContextResult({ source: 'github', sourceId: `${owner}/${repo}#${pr.number}`, title: pr.title, originalDate: pr.created_at, content: pr.body || '', link: pr.html_url, retrievedAt: now() }))
    },
    async getPullRequest ({ owner, repo, number } = {}) {
      const r = await ensure().pulls.get({ owner, repo, pull_number: number })
      const pr = r.data
      return makeContextResult({ source: 'github', sourceId: `${owner}/${repo}#${pr.number}`, title: pr.title, originalDate: pr.created_at, content: pr.body || '', link: pr.html_url, retrievedAt: now() })
    },
    async listBranches ({ owner, repo, per_page = 25 } = {}) {
      const r = await ensure().repos.listBranches({ owner, repo, per_page })
      return (r.data || []).map((b) => makeContextResult({ source: 'github', sourceId: `${owner}/${repo}@${b.name}`, title: b.name, content: (b.commit && b.commit.sha) || '', link: null, retrievedAt: now() }))
    },
    async listCommits ({ owner, repo, sha, per_page = 25 } = {}) {
      const r = await ensure().repos.listCommits({ owner, repo, sha, per_page })
      return (r.data || []).map((c) => makeContextResult({ source: 'github', sourceId: c.sha, title: (c.commit && c.commit.message ? c.commit.message.split('\n')[0] : c.sha), originalDate: c.commit && c.commit.author ? c.commit.author.date : null, content: (c.commit && c.commit.message) || '', link: c.html_url, retrievedAt: now() }))
    },
    async getFileAtRef ({ owner, repo, path, ref } = {}) {
      ...
      return makeContextResult({ source: 'github', sourceId: `${owner}/${repo}:${path}@${ref || 'HEAD'}`, title: path, content, link: (d && d.html_url) || null, retrievedAt: now() })
    }
  }
