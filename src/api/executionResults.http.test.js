...
function buildApp () {
  ...
  const built = createApp({ dispatcher: async () => {}, workerDeps: { artifactStore: store, runner: landmineRunner } })
...
test('traversal / malformed id → 400 and the store is NEVER touched', async () => {
  ...
  const built = createApp({ dispatcher: async () => {}, workerDeps: { artifactStore: landmine, runner: { run: async () => {} } } })
  const server = built.listen(0)
  try {
