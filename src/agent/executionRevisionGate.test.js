...
function harness (over = {}) {
  const calls = { prepare: 0, invoke: 0, test: 0, patchWrites: 0, cleanup: 0, git: [] }
  const audits = []
...
test('D. expectedSha !== observedBaseSha: the run fails with revision_moved', async () => {
  ...
  assert.strictEqual(r.expectedSha, APPROVED)
  assert.strictEqual(r.observedBaseSha, MOVED)
  assert.strictEqual(r.revisionMatch, false)
})
...
test('F/G. a mismatch runs no test command and writes no patch', async () => {
  const h = harness({ baseSha: MOVED })
  await run(h)
  assert.strictEqual(h.calls.test, 0)
