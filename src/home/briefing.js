...
function buildBriefing ({ store, backlog, now }) {
  ...
  const errands = unreadable
    ? { state: 'CANNOT_READ', rows: [], line: '我睇唔到差事紀錄。', checkedAt: t }
    : rows.length
      ? { state: 'HAS_ROWS', rows, line: '', checkedAt: t }
      : { state: 'NONE_RAN', rows: [], line: '今日冇差事跑過。', checkedAt: t }
  ...
  if (unreadable) {
    ...
    waiting = { state: 'CANNOT_READ', cards: [], line: '我睇唔到差事紀錄,所以答唔到你有冇嘢等緊。', checkedAt: t }
  } else {
    const w = rows.filter((e) => e.outcome === OUTCOME.STOPPED_FOR_YOU && !e.resolvedAt)
    waiting = w.length
      ? { state: 'WAITING', cards: w.map((e) => cardFor(e, t)), line: '', checkedAt: t }
      : { state: 'NOTHING_WAITING', cards: [], line: '冇嘢等你決定。', checkedAt: t }
  }

  // ── the Franco line, its own row, off the greeting ──
  let back
  if (!backlog) back = { state: 'NOT_CHECKED', line: '我未睇過 Drive。', checkedAt: t }
  else if (backlog.error) back = { state: 'CANNOT_READ', line: '我睇唔到 Drive 個資料夾(' + backlog.error + ')。', checkedAt: t }
  else if (backlog.empty) back = { state: 'NOTHING', line: 'Drive 度冇等緊處理嘅發票。', checkedAt: backlog.checkedAt || t }
  else back = { state: 'PRESENT', line: backlog.line, checkedAt: backlog.checkedAt || t }

  return { errands, waiting, backlog: back, builtAt: t }
}
