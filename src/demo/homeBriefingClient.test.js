...
test('⛔ the WAITING bar is rendered OUTSIDE the empty-screen guard', () => {
  ...
  assert.doesNotMatch(body, /history\.length > 0/,
    'the waiting bar must NOT be gated on an empty conversation — that is the whole split')
  // And it must actually be called from outside the empty-screen path.
  assert.match(APP_JS, /else renderEmptyScreen\(c\)\r?\n[\s\S]{0,200}renderWaitingBar\(\)/,
    'selectConversation must call it regardless of whether the conversation is empty')
})
